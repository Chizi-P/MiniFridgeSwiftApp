...
struct PostImageCell: View {
    ...
    let images: [String]  //圖片
    
    let width: CGFloat  //螢幕間距
    
    var body: some View {
        
        Group {
            if images.count == 1 {
                loadImage(name: self.images[0]) //func取得圖片
                    .resizable()
                    .scaledToFill()
               ...
               PostImageCellRow(images: images, width: width)
                ...
                VStack(spacing: kImageSpace) {  //水平顯示
